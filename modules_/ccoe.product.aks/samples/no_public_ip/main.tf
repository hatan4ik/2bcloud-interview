module "aks" {
  ...
  # source = "../ccoe.product.aks//module?ref=1.4.0"
  source = "../../module"
  ...
  ebpf_option         = "cilium"
  network_plugin_mode = "overlay" ## Either this set or pod-cide must be specified when network plugin is azure
  outbound_type       = "userDefinedRouting"    ## This is the first step not to create public ip address
  ...
  default_pool = {
    ...
    auto_scaling                 = false
    availability_zones           = []
    enable_node_public_ip        = false        ## This is second step not to create public ip address
